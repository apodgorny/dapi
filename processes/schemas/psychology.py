...
class Authority(O):
	...
	resource  : Resource = O.Field(    description='Resources this authority provides')
...
class Side(O):
	...
	authority             : Authority      = O.Field(    description='The authority figure associated with this side')
	...
	square                : PodgornySquare = O.Field(    description='Key psychological ratios of this side')


class TraumaAttributes(O):
	'''Describes symptomatic patterns and triggers linked to a trauma.'''
	triggers       : list[str] = O.Field(default=[], description='What situations trigger the trauma')
	psychosomatics : list[str] = O.Field(default=[], description='Somatic symptoms linked to trauma')
	key_phrases    : list[str] = O.Field(default=[], description='Internalized traumatic phrases')
...
class TraumaCoping(O):
	...
	resource  : str       = O.Field('',         description='What was used as survival resource')
	actions   : list[str] = O.Field(default=[], description='Typical coping behaviors')
	authority : str       = O.Field('',         description='Who was relied upon for survival')
...
class Trauma(O):
	...
	actors      : list[str]        = O.Field(default=[],                 description='Who was involved')
	since_age   : int              = O.Field(0,                          description='Approximate age when it started')
	attributes  : TraumaAttributes = O.Field(default=TraumaAttributes(), description='Triggers, psychosomatics, phrases')
	components  : TraumaComponents = O.Field(default=TraumaComponents(), description='Core trauma layers')
	coping      : TraumaCoping     = O.Field(default=TraumaCoping(),     description='Survival mechanisms')


class Duality(O):
	'''Represents a core psychological polarity anchored in a trauma.'''
	name   : str    = O.Field('', description='The core polarity name')
	yin    : Side   = O.Field(    description='First side (usually submissive or adaptive)')
	yang   : Side   = O.Field(    description='Second side (usually rebellious or expansive)')
	trauma : Trauma = O.Field(    description='Underlying trauma that anchors the polarity')
...
class Subpersonality(O):
	...
	sides : list[str]   = O.Field([], description='List of Side names that make up this subpersonality')


class Personality(O):
	'''Represents the whole psyche as a network of subpersonalities.'''
	subpersonalities : list[Subpersonality] = O.Field(default=[], description='All inner subpersonalities')
